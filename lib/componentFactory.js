  ...
  switch (element[0].name) {
    ...
    case this.components.row:
      ...
      if (element.attr('bgcolor')) {
        bgcolor = 'bgcolor="' + element.attr('bgcolor') + '"';
      }

      return format('<table %s class="%s" '+bgcolor+'><tbody><tr>%s</tr></tbody></table>', attrs, classes.join(' '), inner);
    ...
    case this.components.container:
      ...
      return format('<table %s align="center" class="%s" '+bgcolor+'><tbody><tr><td>%s</td></tr></tbody></table>', attrs, classes.join(' '), inner);
    ...
    case this.components.spacer:
      ...
      if (element.attr('width')) {
        width = 'width="' + element.attr('width') + '"';
      }
      if (element.attr('class')) {
        classes = classes.concat(element.attr('class').split(' '));
      }
      if (element.attr('size-sm') || element.attr('size-lg')) {
        if (element.attr('size-sm')) {
          ...
          html += format('<table %s class="%s hide-for-large" '+bgcolor+' '+width+'><tbody><tr><td height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&nbsp;</td></tr></tbody></table>', attrs);
        }
        if (element.attr('size-lg')) {
          size = (element.attr('size-lg'));
          html += format('<table %s class="%s show-for-large" '+bgcolor+' '+width+'><tbody><tr><td height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&nbsp;</td></tr></tbody></table>', attrs);
        }
      } else {
        size = (element.attr('size')) || 16;
        html += format('<table %s class="%s" '+bgcolor+' '+width+'><tbody><tr><td height="'+size+'px" style="font-size:'+size+'px;line-height:'+size+'px;">&nbsp;</td></tr></tbody></table>', attrs);
      }
